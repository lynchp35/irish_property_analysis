...
if __name__ == "__main__":
    ...
    daft_listings = pd.read_csv("data/daft_from_page_0_till_page_987_by_20.csv", index_col=["Unnamed: 0"])
    
    daft_listings["Price"] = daft_listings["Price"].apply(lambda x :fix_price(x))
    daft_listings["County"] = daft_listings["Address"].apply(lambda x :find_county(x, county_dict))
    daft_listings.dropna(subset="County", inplace=True)
    daft_listings["Province"] = daft_listings["County"].apply(lambda x: find_province(x, county_dict))
